...
export async function GET() {
  try {
    ...
    const clothes = await Cloth.find({ userEmail: session.user.email }).sort({
      createdAt: -1,
    });
  ...
  try {
    ...
    const groqResponse = await fetch(
      "https://api.groq.com/openai/v1/chat/completions",
      {
        ...
        body: JSON.stringify({
          // YENİ SATIR:
          model: "meta-llama/llama-4-scout-17b-16e-instruct", // Groq'un süper hızlı görsel modeli
          messages: [
            {
              role: "user",
              content: [
    ...
    if (!groqResponse.ok) {
      ...
      console.error("GROQ'DAN GELEN ASIL HATA:", errorText);
    ...
    return NextResponse.json(
      { error: "Kıyafet eklenemedi veya analiz edilemedi" },
      { status: 500 },
    );
  }
}
